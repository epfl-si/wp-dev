...
# of result (`n=200`) is maxed at 100. Using `?last=2026-300` is a quick fix as
# it's not possible to sort them in reverse order or to increase the limit.
_get_latest_image_tag = ./devscripts/curl-authenticated-quay '/v2/svc0041/wp-php/tags/list?last=2026-300' | \
...
.PHONY: checkout
checkout: src wp-ops wp-operator wp-clinic wp-continuous-integration wp-controller menu-api ## Checkout wp-ops, wp-operator, wp-clinic, wp-continuous-integration, wp-controller, menu-api, WP Themes and WP Plugins
...
wp-controller:
	$(call _git_clone, epfl-si/wp-controller)
