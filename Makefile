...
DOCKER_IMAGE_STAMPS = .docker-images-pulled.stamp \
  ...
  .docker-mgmt-image-built.stamp \
  .docker-httpd-image-built.stamp
...
$(WP_CONTENT_DIR): .docker-httpd-image-built.stamp $(JAHIA2WP_DIR)
...
.docker-httpd-image-built.stamp: .docker-base-image-built.stamp wp-ops \
                                 $(_DOCKER_HTTPD_IMAGE_DEPS)
	docker build -t $(DOCKER_HTTPD_IMAGE_NAME) wp-ops/docker/httpd
	touch $@

.docker-mgmt-image-built.stamp: .docker-base-image-built.stamp wp-ops \
                                 $(_DOCKER_MGMT_IMAGE_DEPS)
	docker build -t $(DOCKER_MGMT_IMAGE_NAME) wp-ops/docker/mgmt
	touch $@
